...
## Streamlit Cloud 배포 (미완료 — 이어서 진행)

1. https://share.streamlit.io → GitHub 연동
2. Repository: `elova9049/lab-arng`
...
5. Deploy 후 생성 URL을 팀에 공유

**참고:** 코드는 이미 GitHub `main`에 push됨. 배포 화면에서 `This branch does not exist`가 나오면 GitHub App 권한([settings/installations](https://github.com/settings/installations) → Streamlit → `lab-arng` 저장소 허용) 확인 후 저장소 URL을 다시 붙여넣기.
...
1. **Streamlit Cloud 배포 완료** — 외부 URL 공유
